...
func (l *DistributedLog) setupRaft(dataDir string) error {
	...
	// configure initial offset to 1 as required by Raft
	logConfig := l.config
...
func (l *DistributedLog) apply(reqType RequestType, req proto.Message) (interface{}, error) {
	...
	// future.Response() returns what FSM's apply method returned
	res := future.Response()
...
// Join adds server to the Raft cluster
...
// WaitForLeader blocks until the cluster has elected leader or times out
...
// GetServers converts the data from Raft's raft.Server type into *api.Server type
...
// Apply Reft invokes this method after commiting a log entry
...
// in this case the snapshot is FSM's log
...
// Restore Raft calls this to restore an FSM from a snapshot
...
func NewStreamLayer(ln net.Listener, serverTLSConfig, peerTLSConfig *tls.Config) *StreamLayer {
	return &StreamLayer{
		ln:              ln,
		serverTLSConfig: serverTLSConfig, // to accept incoming connections
		peerTLSConfig:   peerTLSConfig,   // to create outgoing connections
	}
}
