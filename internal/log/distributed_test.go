...
func TestMultipleNodes(t *testing.T) {
	...
	for i := 0; i < nodeCount; i++ {
		...
		// the first server bootstraps the cluster and becomes the leader
		if i == 0 {
	...
	// test replication by appeding some records to our leader server
	// and check that Raft replicated the records to its followers
	records := []*api.Record{
		{Value: []byte("first")},
		{Value: []byte("second")},
	}
	...
	// check that leader stops replicating to a server that's left the cluster
